...
public static class CollectionCqrsEndpointExtensions
{
    public static IEndpointRouteBuilder MapCollectionCqrsEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        //
        // CACHE NOTE (Day-21): this route intentionally bypasses HybridCache
        // to preserve its value as a raw-latency benchmark.  It always hits
        // the DB directly and therefore always returns the current DB state,
        // while /{id} and /{id}/ef may serve a cached snapshot for up to 30 s
        // after a write.  Do not use this route as the authoritative read path
        // in production — use /{id} instead.
