...
export class QuotesListComponent {
  ...
  // Friendly message for a list-load failure — mapped AppError.message from the
  // errorInterceptor, falling back defensively (httpResource.error() is unknown).
  protected readonly listErrorMessage = computed<string>(() => {
    const e = this.quotes.listError();
    return e instanceof AppError ? e.message : 'Failed to load quotes. Is the Week-1 API running on :5075?';
  });
}
