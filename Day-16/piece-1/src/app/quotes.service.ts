...
export class QuotesService {
  ...
  // The errorInterceptor maps every API failure to a typed AppError whose
  // .message is already friendly (derived from the server's ProblemDetails),
  // so consumers just read it. The HttpErrorResponse branch is a fallback for
  // the (unexpected) case an error reaches here unmapped — no `any` used.
