...
@Component({
  ...
  styles: [`
    ...
    .signed-in button { background: #565d64; }
    .error { color: #b02a37; font-size: 0.8rem; margin: 0.25rem 0 0; }
    .hint { font-size: 0.75rem; color: #565d64; margin: 0.4rem 0 0; }
  `],
})
export class AuthBarComponent {
  ...
  protected async onSubmit(): Promise<void> {
    ...
    try {
      ...
      // The errorInterceptor maps the 401 ProblemDetails → AppError whose
      // .message is the server's "Invalid email or password." detail.
      this.error.set(
