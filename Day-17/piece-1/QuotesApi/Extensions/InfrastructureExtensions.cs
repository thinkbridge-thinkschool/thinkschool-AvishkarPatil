        ...
        IConfiguration configuration)
    {
        ...
        services
            ...
            .Configure<IOptions<EntraIdOptions>>((bearer, entra) =>
            {
                ...
                // Day-17: validate the Managed-Identity access token the broker forwards.
                // Accept the App ID URI and the bare app/client id as audience, both
                // v2/v1 issuers, and read app roles from the `roles` claim.
                bearer.TokenValidationParameters = new TokenValidationParameters
                {
                    ValidateIssuer           = true,
                    ValidateAudience         = true,
                    ValidateLifetime         = true,
                    ValidateIssuerSigningKey = true,
                    ValidAudiences           = new[] { e.Audience, e.ClientId },
                    ValidIssuers             = new[]
                    {
        ...
        services.AddAuthorization(options =>
        {
            ...
            // Day-17: the data-read endpoints require a valid Managed-Identity
            // token carrying the Quotes.Access app role. The MultiAuth scheme
            // forwards the (v2) Entra token to the EntraId JwtBearer, which
            // validates signature/issuer/audience; this policy then checks the
            // app role. A missing/invalid token → 401; a valid token without the
            // role → 403.
            options.AddPolicy("mi-read", p =>
            {
                p.RequireAuthenticatedUser();
                p.RequireAssertion(ctx => ctx.User.Claims.Any(c =>
                    (c.Type == "roles" || c.Type == System.Security.Claims.ClaimTypes.Role)
                    && c.Value == "Quotes.Access"));
            });
