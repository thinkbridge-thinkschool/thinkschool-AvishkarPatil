...
public sealed class QuotePublisher : IQuotePublisher, IAsyncDisposable
{
    ...
    // Auto-generated MessageId overload — used by direct publish calls.
    public Task PublishAsync(QuoteCreatedMessage message, CancellationToken ct = default)
        => PublishAsync(message, Guid.NewGuid().ToString(), ct);

    // Caller-supplied MessageId overload — used by OutboxRelayWorker so that
    // every re-delivery attempt carries the same stable id stored in the outbox row.
    public async Task PublishAsync(QuoteCreatedMessage message, string messageId, CancellationToken ct = default)
    {
