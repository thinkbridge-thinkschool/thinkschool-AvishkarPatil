...
public static class InfrastructureExtensions
{
        ...
        IConfiguration configuration)
    {
        ...
        // ── Day-22: fault injection handler ──────────────────────────────────
        // Transient: each HttpClient creation gets a fresh instance.
        // The Enabled flag is static so toggling it from an API endpoint is
        // visible to ALL requests immediately.
        services.AddTransient<FaultInjectionHandler>();

        // Outbound HTTP to Entra ID (OIDC metadata fetch).
        //
        // Handler chain (outer → inner):
        //   [0] Polly resilience pipeline  ← AddResilienceHandler called first on the builder
        //   [1] FaultInjectionHandler      ← AddHttpMessageHandler called second on the builder
        //   [*] HttpClientHandler          ← primary handler (real network)
        //
        // Polly sees the 503 returned by FaultInjectionHandler and applies
        // its retry / circuit-breaker logic exactly as it would for real failures.
        //
        // NOTE: AddResilienceHandler returns IHttpResiliencePipelineBuilder, not IHttpClientBuilder,
        // so AddHttpMessageHandler cannot be chained on it.  We capture IHttpClientBuilder first
        // and call both methods on the same variable — order of calls determines chain position.
        var entraIdClientBuilder = services
        ...
        entraIdClientBuilder.AddResilienceHandler("default", (pipeline, ctx) =>
        {
        ...
        // FaultInjectionHandler is added after AddResilienceHandler so it sits INSIDE the
        // Polly pipeline — Polly intercepts the 503 response it returns.
        entraIdClientBuilder.AddHttpMessageHandler<FaultInjectionHandler>();
