...
@Component({
  ...
  styles: [`
    ...
    .success { font-size: 0.85rem; color: var(--success); font-weight: 600; }
    button { font: inherit; padding: 0.5rem 1.1rem; border: 0; border-radius: 6px;
             background: var(--primary); color: #fff; cursor: pointer;
             transition: background 0.15s; }
    button:hover:not(:disabled) { background: var(--primary-dark); }
    button:disabled { background: var(--text-muted); cursor: progress; }
...
export class QuoteFormSignalsComponent {
  ...
  protected async onSubmit(): Promise<void> {
    ...
    const ok = await submit(this.form, {
      ...
      action: async (f) => {
        ...
        try {
          ...
          this.toastSvc.show(message, 'error');
    ...
    if (ok) {
      ...
      this.toastSvc.show('Quote created successfully', 'success');
